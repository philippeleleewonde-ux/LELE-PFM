...
export function diagnoseEmployeePerformance(
  ...
  const bulletinEntry = bulletinData?.find(
    (e: any) => e.employeeName?.toLowerCase().includes(employeeName.toLowerCase())
  );
  ...
  if (bulletinEntry) {
    ...
    if (bulletinEntry.indicators) {
      Object.entries(bulletinEntry.indicators).forEach(([key, ind]: [string, any]) => {
  ...
  const employeeCostEntries = costEntries?.filter(
    (e: any) => e.employee_name?.toLowerCase().includes(employeeName.toLowerCase())
  );
  ...
  if (employeeCostEntries && employeeCostEntries.length > 0) {
    ...
    kpiTypes.forEach(kpi => {
      const kpiEntries = employeeCostEntries.filter((e: any) => e.kpi_type === kpi);
      const totalFrais = kpiEntries.reduce((sum: number, e: any) => sum + (e.compensation_amount || 0), 0);
...
export function setupGlobalDiagnostic() {
  if (typeof window !== 'undefined') {
    (window as any).diagPerformance = (employeeName: string) => {
