...
import { THEME_NAMES } from '@/modules/module2/engine/constants';
...
function DeltaIndicator({ value, inverted = false, suffix = '' }: { value: number | null; inverted?: boolean; suffix?: string }) {
  ...
  // For satisfaction scale (1=best, 5=worst), a negative delta = improvement
...
function ragColor(score: number, type: 'global' | 'participation' | 'enps'): string {
  if (type === 'global') {
    // Satisfaction: 1=best, 5=worst. Low score = good
...
function heatmapLabel(delta: number): string {
  // On satisfaction scale: negative delta = improvement (score went down = better)
...
function getQuestionText(code: string): string {
  const q = SURVEY_QUESTIONS.find(q => q.code === code);
  return q ? q.question : code;
}

function getQuestionTheme(code: string): string {
  const q = SURVEY_QUESTIONS.find(q => q.code === code);
  return q ? q.themeId : 'theme1';
}
...
export function TrendsDashboard() {
  ...
  const { campaigns, loading: campaignsLoading } = useCampaigns(false);
  const {
    snapshots,
    loading,
    latestSnapshot,
    ...
    campaignCount,
    getSparklineData,
  } = useTrendsData(selectedCampaignId);
      ...
      <Card>
        <CardContent className="pt-12 pb-12 text-center">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            ...
            les résultats d'au moins une campagne de satisfaction.
          </p>
  ...
  const chartData = snapshots.map((s, idx) => {
    ...
    return {
      ...
      theme1: s.theme_scores?.theme1 || 0,
      theme2: s.theme_scores?.theme2 || 0,
      theme3: s.theme_scores?.theme3 || 0,
      theme4: s.theme_scores?.theme4 || 0,
    };
  });

  // --- Build scatter data for impact matrix ---
  ...
  if (latestSnapshot?.question_correlations) {
    ...
    // Get per-question avg scores from latest snapshot themes
    // We need to calculate from available data
    for (const [code, correlation] of Object.entries(correlations)) {
      if (correlation === 0) continue;
      const theme = getQuestionTheme(code);
      // We approximate the question score from the theme score
      // In real implementation, this would come from question-level data
      const themeScore = latestSnapshot.theme_scores?.[theme] || 3;
      // Add some variance based on correlation for visual spread
      const questionScore = themeScore + (correlation - 0.5) * 0.5;
  ...
  if (deltas && latestSnapshot) {
    // Theme degradation ≥ 0.5 pts (score increased = worse)
    for (const [themeId, delta] of Object.entries(deltas.themes)) {
      if (delta >= 0.5) {
        alerts.push({
          type: 'error',
          message: `Dégradation : "${THEME_NAMES[themeId]}" a augmenté de +${delta.toFixed(1)} pts (score plus élevé = moins satisfait)`,
        });
      }
    }

    // Critical score (theme > 3.5 = insatisfaction)
    if (latestSnapshot.theme_scores) {
      for (const [themeId, score] of Object.entries(latestSnapshot.theme_scores)) {
        if (score >= 3.5) {
          alerts.push({
            type: 'error',
            message: `Score critique : "${THEME_NAMES[themeId]}" à ${score.toFixed(1)}/5 (seuil d'alerte : 3.5)`,
          });
        }
      }
    }
    ...
    if (latestSnapshot.by_department) {
      for (const dept of latestSnapshot.by_department) {
        if (dept.participationRate > 0 && dept.participationRate < 50) {
          alerts.push({
            type: 'warning',
            message: `Participation faible : "${dept.name}" à ${dept.participationRate.toFixed(0)}% (seuil : 50%)`,
          });
        }
      }
    }

    // Inter-department gap
      ...
      for (const themeId of Object.keys(THEME_NAMES)) {
        ...
        if (scores.length >= 2) {
          ...
          if (gap > 1.5) {
            alerts.push({
              type: 'info',
              message: `Écart inter-départements de ${gap.toFixed(1)} pts sur "${THEME_NAMES[themeId]}"`,
            });
          }
        }
      }
    ...
    <div className="space-y-6">
      ...
      {selectedCampaignId && latestSnapshot && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground bg-accent/50 rounded-lg px-4 py-2">
          ...
          <span>
            ...
            {' — '}
            <button
              className="text-[#FF4530] hover:underline font-medium"
        ...
        <Card className={`border ${latestSnapshot ? ragColor(latestSnapshot.global_score, 'global') : ''}`}>
          <CardContent className="pt-5 pb-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Score Global</span>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Score moyen par thème sur {campaignCount} campagne{campaignCount > 1 ? 's' : ''} •
            Échelle : 1 (Satisfait Pleinement) → 5 (Pas satisfait du tout) •
            Objectif : ≤ {OBJECTIVE_LINE}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {chartData.length >= 1 ? (
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={chartData} margin={{ top: 5, right: 30, left: 5, bottom: 5 }}>
                ...
                <ReferenceLine
                  ...
                  stroke="#9ca3af"
                  strokeDasharray="6 4"
                  label={{ value: `Objectif ${OBJECTIVE_LINE}`, position: 'right', style: { fontSize: 10, fill: '#9ca3af' } }}
                />
                <Line
                  type="monotone"
                  dataKey="global"
                  ...
                  activeDot={{ r: 7 }}
                />
                {Object.entries(THEME_NAMES).map(([themeId, themeName]) => (
                  <Line
                    key={themeId}
                    type="monotone"
                    dataKey={themeId}
                    name={themeName}
                    stroke={THEME_COLORS[themeId]}
                    strokeWidth={1.5}
                    dot={{ r: 3 }}
      ...
      {/* ================================================================== */}
      {/* ZONE 3 — Heatmap Départements × Thèmes                             */}
      {/* ================================================================== */}
                ...
                <thead>
                  <tr className="border-b">
                    ...
                    {Object.values(THEME_NAMES).map(name => (
                      <th key={name} className="text-center py-2 px-3 font-medium text-muted-foreground">{name}</th>
                    ))}
                    ...
                    .map(dept => {
                      ...
                      return (
                        <tr key={dept.name} className="border-b last:border-0">
                          ...
                          {Object.keys(THEME_NAMES).map(themeId => {
                            const delta = deptDeltas?.themes[themeId] ?? 0;
                            return (
                              <td key={themeId} className="text-center py-2 px-1">
      ...
      {scatterData.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Chaque point = une question • X = score moyen • Y = impact sur la satisfaction globale (corrélation)
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex flex-wrap gap-4 mt-4 justify-center">
              {Object.entries(THEME_NAMES).map(([themeId, themeName]) => (
                <div key={themeId} className="flex items-center gap-2 text-xs">
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: THEME_COLORS[themeId] }} />
                  {themeName}
                </div>
              ))}
            </div>
        ...
        <Card className="border-dashed">
          <CardContent className="pt-6 text-center text-muted-foreground">
            ...
            <p className="text-sm">
              ...
              <strong> 2 campagnes</strong> de satisfaction.
            </p>
