...
import {
  ...
  RPS_SECTIONS,
  type SurveyOption,
} from '@/modules/module5/data/riskQuestions';

// ============================================================================
// PSYCHOSOCIAL SURVEY — Full-page dark premium questionnaire
// Design: slate-900 bg, #7c3aed accent (violet), circular progress, trust badges
// Modes: embedded (dans Module5DemoPage / Module5Dashboard)
// ============================================================================

const ACCENT = '#7c3aed';

interface PsychosocialSurveyProps {
  onBack?: () => void;
  bypassAccessKey?: boolean;
  surveyId?: string | null;
}

export default function PsychosocialSurvey({ onBack, bypassAccessKey, surveyId: propSurveyId }: PsychosocialSurveyProps) {
  // --- State -----------------------------------------------------------------
  ...
  const [surveyId] = useState<string | null>(propSurveyId || null);
  ...
  const questions = useMemo(() => {
    if (businessLines.length === 0) return RPS_QUESTIONS;
    return RPS_QUESTIONS.map(q => {
      if (q.code === 'D1') {
        return {
          ...q,
  ...
  const currentSection =
    phase === 'theme-intro'
      ? RPS_SECTIONS.find(s => s.id === pendingThemeId)
      : RPS_SECTIONS.find(s => s.id === currentQuestion?.themeId);
  ...
  const handleStart = async () => {
    ...
    // Validate access key against rps_access_keys
    const upperKey = accessKey.toUpperCase().trim();
    const { data, error } = await supabase
      .from('rps_access_keys' as any)
      .select('id, is_used')
    ...
    if (error || !data) {
      setAccessKeyError("Clé d'accès invalide");
      return;
    }

    if (data.is_used) {
      setAccessKeyError('Vous avez déjà répondu à ce questionnaire');
      return;
    }
  ...
  const submitAnswers = async (finalAnswers: Record<string, number | string>) => {
    if (!surveyId) return;
    try {
      await (supabase.from('rps_survey_responses' as any) as any).insert({
        survey_id: surveyId,
        responses: finalAnswers as unknown as Record<string, unknown>,
      });
    } catch (err) {
      console.error('RPS Survey submit error:', err);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-sans selection:bg-violet-500 selection:text-white overflow-hidden relative">
      ...
      <div
        ...
        style={{
          backgroundImage:
            'radial-gradient(circle at 20% 30%, rgba(124,58,237,0.15) 0%, transparent 50%), radial-gradient(circle at 80% 70%, rgba(59,130,246,0.1) 0%, transparent 50%)',
        }}
      />

      {/* --- Top accent bar --- */}
      <div className="relative z-20 w-full h-1.5 bg-[#7c3aed] shadow-[0_0_20px_rgba(124,58,237,0.5)]" />
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 lg:py-12 flex flex-col min-h-[calc(100vh-6px)]">
        ...
        <div className="flex items-center justify-between mb-12 lg:mb-0 lg:absolute lg:top-8 lg:left-8 lg:right-8">
          ...
          <div className="flex items-center gap-2 text-white/80 text-sm uppercase tracking-widest font-semibold">
            ...
            LELE HCM — Risques Psychosociaux
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center max-w-6xl mx-auto w-full flex-1">
          ...
          <div className="lg:col-span-7 flex flex-col gap-6 lg:pr-12">
            ...
            {/* ---- WELCOME ---- */}
            {phase === 'welcome' && (
              <div key="welcome" className="space-y-6" style={{ animation: 'rpsFadeIn 0.6s ease-out' }}>
                <h1 className="text-3xl lg:text-5xl font-bold text-white leading-tight">
                  Évaluez les{' '}
                  <span className="text-[#7c3aed] inline-block border-b-4 border-[#7c3aed]/30 pb-1">
                    Risques Psychosociaux
                  </span>{' '}
                  de votre environnement
                </h1>
                <p className="text-slate-300 text-lg lg:text-xl font-light">
                  Participez à cette enquête <span className="text-[#7c3aed]">100% anonyme</span> pour
                  contribuer à la prévention des risques psychosociaux au travail.
                </p>
                <div className="space-y-2 text-slate-400 text-sm">
                  <p>• 21 questions réparties en 6 axes psychosociaux</p>
                  <p>• Durée estimée : 3–5 minutes</p>
                ...
                {!bypassAccessKey && surveyId && (
                  <div className="mt-4 space-y-3 max-w-md">
                    ...
                    <input
                      ...
                      maxLength={6}
                      className="w-full px-5 py-4 bg-white/5 border border-slate-600 rounded-xl text-white text-lg font-mono tracking-[0.3em] text-center placeholder:text-slate-600 placeholder:tracking-[0.3em] focus:outline-none focus:ring-2 focus:ring-[#7c3aed] focus:border-transparent transition-all"
                    />
                    {accessKeyError && (
                ...
                <button
                  ...
                  className="mt-4 px-10 py-5 bg-[#7c3aed] hover:bg-[#8b5cf6] text-white rounded-full transition-all duration-300 transform hover:translate-x-1 font-bold text-lg shadow-lg shadow-[#7c3aed]/30 flex items-center gap-3 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-[#7c3aed] disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            {phase === 'theme-intro' && currentSection && (
              <div
                ...
                className="space-y-6"
                style={{ animation: 'rpsFadeIn 0.6s ease-out' }}
              >
                <p className="text-[#7c3aed] text-sm font-medium uppercase tracking-wider">
                  Axe {currentSection.number} sur 6
                </p>
                ...
                <p className="text-slate-500">{currentSection.questionCount} question{currentSection.questionCount > 1 ? 's' : ''} dans cette section</p>
                <button
                  onClick={handleContinueFromIntro}
                  className="mt-4 px-10 py-5 bg-[#7c3aed] hover:bg-[#8b5cf6] text-white rounded-full transition-all duration-300 transform hover:translate-x-1 font-bold text-lg shadow-lg shadow-[#7c3aed]/30 flex items-center gap-3 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-[#7c3aed]"
                >
                  Continuer
            ...
            {phase === 'survey' && currentQuestion && (
              <div
                key={`q-${currentIndex}`}
                className="space-y-4"
                style={{ animation: 'rpsFadeIn 0.5s ease-out' }}
              >
                  ...
                  {currentQuestion.options.map((option: SurveyOption, idx: number) => (
                    <button
                      ...
                      className={`
                        ...
                        focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-[#7c3aed]
                        ${
                          option.type === 'primary'
                            ? 'bg-[#7c3aed] hover:bg-[#8b5cf6] text-white shadow-lg shadow-[#7c3aed]/30 font-bold'
            ...
            {/* ---- COMPLETED ---- */}
            {phase === 'completed' && (
              <div key="completed" style={{ animation: 'rpsFadeIn 0.6s ease-out' }}>
                <div className="bg-white/5 p-8 rounded-3xl border border-white/10 backdrop-blur-sm space-y-6">
                  ...
                  <p className="text-slate-300 text-lg">
                    ...
                    prévenir les risques psychosociaux et améliorer votre bien-être au travail.
                  </p>
                  <div className="flex flex-wrap gap-3">
                    <span className="bg-white/5 rounded-xl px-4 py-2 text-slate-400 text-sm">
                      21 questions complétées
                    </span>
                    <span className="bg-white/5 rounded-xl px-4 py-2 text-slate-400 text-sm">
                      6 axes analysés
                    </span>
          ...
          <div className="lg:col-span-5 flex flex-col items-center justify-center relative">
            ...
            <div
              ...
              style={{ animation: 'rpsSpin 60s linear infinite' }}
            />
            ...
            <div className="relative w-64 h-64 mb-12 group cursor-default">
              <div className="absolute inset-0 bg-[#7c3aed]/5 rounded-full blur-3xl transform group-hover:scale-110 transition-transform duration-700" />
              <svg className="w-full h-full transform -rotate-90 drop-shadow-2xl">
                <circle cx="50%" cy="50%" r={radius} stroke="currentColor" strokeWidth="12" fill="transparent" className="text-slate-700/50" />
                <circle cx="50%" cy="50%" r={radius} stroke="#ffffff" strokeWidth="12" fill="transparent" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" className="transition-all duration-1000 ease-out" />
                <circle cx="50%" cy="50%" r={radius} stroke={ACCENT} strokeWidth="12" fill="transparent" strokeDasharray={`${circumference * 0.01} ${circumference}`} strokeDashoffset={strokeDashoffset} strokeLinecap="round" className="transition-all duration-1000 ease-out" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
                <span className="text-5xl font-bold tracking-tighter transition-all duration-500">{progress}%</span>
            ...
            <div className="flex flex-col gap-4 w-full max-w-xs relative z-10">
              <p className="text-center text-slate-400 text-xs mb-2">
                100% Anonyme & Sécurisé • Prévention RPS
              </p>

              <div className="bg-[#1e293b]/80 backdrop-blur-md p-4 rounded-xl border border-slate-700 flex items-center gap-4 hover:border-violet-500/50 transition-colors group">
                <div className="w-10 h-10 rounded-lg bg-violet-500/20 flex items-center justify-center text-violet-400 group-hover:text-violet-300 group-hover:scale-110 transition-all">
              ...
              <div className="bg-[#1e293b]/80 backdrop-blur-md p-4 rounded-xl border border-slate-700 flex items-center gap-4 hover:border-green-500/50 transition-colors group">
                ...
                <div>
                  <div className="text-white font-bold text-lg leading-none">Prévention</div>
