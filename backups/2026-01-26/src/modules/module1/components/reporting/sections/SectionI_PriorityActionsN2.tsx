...
/**
 * Section I: Priority Actions N+2
 * Source: Reproduction de la Page 15 (Priority Actions N+2)
 * Affiche le plan d'action prioritaire pour l'année N+2 : Indicateurs & Distribution par ligne
 */
export function SectionI_PriorityActionsN2({ calculated, selectedCurrency, businessLines }: SectionIProps) {
    ...
    // PPR total pour N+2 (= gainsN2 depuis feuille 9-PLANIFICATION BUDGET-3ANS)
    const pprN2 = calculated.gainsN2 || 0;

    // Vérifier si les données upstream sont complètes
    const hasCompleteData = pprN2 > 0 && businessLines.length > 0;
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex items-center gap-3 pb-3 border-b border-border">
                ...
                <div>
                    <h3 className="text-xl font-bold text-foreground">Section I - Priority Actions N+2</h3>
                    <p className="text-sm text-muted-foreground">Plan d'action prioritaire par domaines clés d'amélioration socio-économique (Année N+2)</p>
                </div>
            </div>
                ...
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-destructive/80">
                            Impossible de calculer la distribution. Veuillez vérifier que le PPR N+2 est calculé (Page 9) et que les lignes d'activité sont configurées.
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-card rounded-xl p-6 border border-border shadow-sm">
                    ...
                    <div className="text-3xl font-bold text-green-600 dark:text-green-400">{formatCurrency(pprN2, selectedCurrency)}</div>
            ...
            {/* ========== SECTIONS TRIMESTRIELLES N+2 ========== */}
            {hasCompleteData && (
                <>
                    {[1, 2, 3, 4].map(quarter => {
                        const quarterPPR = pprN2 / 4;
                        ...
                        return (
                            <React.Fragment key={quarter}>
                                ...
                                <div className="flex items-center gap-3 pb-3 border-b border-border mt-10">
                                    ...
                                    <div>
                                        <h3 className="text-xl font-bold text-foreground">Trimestre T{quarter} - N+2</h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="bg-card rounded-xl p-6 border border-border shadow-sm">
                                        <div className="text-sm font-bold text-muted-foreground mb-1 flex items-center gap-2">
                                            PPR T{quarter} N+2
                                            <SmartQuarterDateWidget yearsOffset={2} quarter={quarter as 1 | 2 | 3 | 4} showLockIcon={true} allowLockToggle={true} className="text-xs" />
                                        </div>
                                        <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">{formatCurrency(quarterPPR, selectedCurrency)}</div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-6">
                                        {quarterDistributions.map((line, idx) => (
                                            <div key={idx} className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                                                <div className="bg-purple-500/10 p-4 border-b border-border flex justify-between items-center">
                                                    <div>
                                                        <h5 className="font-bold text-foreground text-lg">{line.lineName}</h5>
                                                        <div className="text-xs text-muted-foreground flex gap-3 mt-1">
                                                            <span>Staff: <strong>{line.staffCount}</strong></span>
                                                            ...
                                                            {indicators.map((indicator) => {
                                                                ...
                                                                return (
                                                                    <tr key={indicator.id} className="border-b border-border/50 last:border-0 hover:bg-muted/20 transition-colors">
                                                                        ...
                                                                        <td className="text-right py-3 px-4 font-mono text-purple-600 dark:text-purple-400 font-bold">
                                                                            {formatCurrency(dist?.perPerson || 0, selectedCurrency)}
                                                                        </td>
                                                                    </tr>
                                                                );
