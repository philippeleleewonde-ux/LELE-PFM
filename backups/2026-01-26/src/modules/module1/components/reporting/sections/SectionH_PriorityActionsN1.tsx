...
/**
 * Section H: Priority Actions N+1
 * Source: Reproduction de la Page 14 (Priority Actions N+1)
 * Affiche le plan d'action prioritaire pour l'année N+1 : Indicateurs & Distribution par ligne
 */
export function SectionH_PriorityActionsN1({ calculated, selectedCurrency, businessLines }: SectionHProps) {
    ...
    // PPR total pour N+1 (= gainsN1 depuis feuille 9-PLANIFICATION BUDGET-3ANS)
    const pprN1 = calculated.gainsN1 || 0;

    // Vérifier si les données upstream sont complètes
    const hasCompleteData = pprN1 > 0 && businessLines.length > 0;
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex items-center gap-3 pb-3 border-b border-border">
                ...
                <div>
                    <h3 className="text-xl font-bold text-foreground">Section H - Priority Actions N+1</h3>
                    <p className="text-sm text-muted-foreground">Plan d'action prioritaire par domaines clés d'amélioration socio-économique</p>
                </div>
            </div>
                ...
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-destructive/80">
                            Impossible de calculer la distribution. Veuillez vérifier que le PPR N+1 est calculé (Page 9) et que les lignes d'activité sont configurées.
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-card rounded-xl p-6 border border-border shadow-sm">
                    <div className="text-sm font-bold text-muted-foreground mb-1 flex items-center gap-2">
                        PPR Year N+1
                        <SmartDateWidget yearsOffset={1} showIcon={false} showLockIcon={true} allowLockToggle={true} className="text-xs" />
                    </div>
                    <div className="text-3xl font-bold text-green-600 dark:text-green-400">{formatCurrency(pprN1, selectedCurrency)}</div>
            ...
            {/* ========== SECTIONS TRIMESTRIELLES N+1 ========== */}
            {hasCompleteData && (
                <>
                    {[1, 2, 3, 4].map(quarter => {
                        const quarterPPR = pprN1 / 4;
                        ...
                        return (
                            <React.Fragment key={quarter}>
                                ...
                                <div className="flex items-center gap-3 pb-3 border-b border-border mt-10">
                                    ...
                                    <div>
                                        <h3 className="text-xl font-bold text-foreground">Trimestre T{quarter} - N+1</h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="bg-card rounded-xl p-6 border border-border shadow-sm">
                                        <div className="text-sm font-bold text-muted-foreground mb-1 flex items-center gap-2">
                                            PPR T{quarter} N+1
                                            <SmartQuarterDateWidget yearsOffset={1} quarter={quarter as 1 | 2 | 3 | 4} showLockIcon={true} allowLockToggle={true} className="text-xs" />
                                        </div>
                                        <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">{formatCurrency(quarterPPR, selectedCurrency)}</div>
                                                            ...
                                                            {indicators.map((indicator) => {
                                                                ...
                                                                return (
                                                                    <tr key={indicator.id} className="border-b border-border/50 last:border-0 hover:bg-muted/20 transition-colors">
                                                                        ...
                                                                        <td className="text-right py-3 px-4 font-mono text-blue-600 dark:text-blue-400 font-bold">
                                                                            {formatCurrency(dist?.perPerson || 0, selectedCurrency)}
                                                                        </td>
                                                                    </tr>
                                                                );
